...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let numLetters = settings[kNumLettersKey] as? Int else {
          assertionFailure("Expected an Integer Value, got NIL!")
          return
      }
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      ...
      guard let numGuesses = settings[kNumGuessesKey] as? Int else {
          assertionFailure("Expected an Int value, got NIL!")
          return
      }
    ...
    // START YOUR CODE HERE
      // START YOUR CODE HERE
        guard let themeString = settings[kWordThemeKey] as? String,
              let themeEnum = WordTheme(rawValue: themeString) else {
            assertionFailure("Invalid theme value")
            return
        }

        goalWord = WordGenerator.generateGoalWord(with: themeEnum)
        // END YOUR CODE HERE
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let alien = settings[kIsAlienWordleKey] as? Bool else {
          assertionFailure("Expected a Bool, got NIL!")
          return
      }
